...
export default function Dashboard() {
    ...
    useEffect(() => {
        axios.get('https://192.168.239.148:8080/v1/feeds')
            .then(response => {
    ...
    useEffect(() => {
        ...
        axios.get('https://192.168.239.148:8080/v1/feed_follows', {
            headers: {
    ...
    useEffect(() => {
        const apiKey = localStorage.getItem('apiKey');
        axios.get('https://192.168.239.148:8080/v1/posts', {
            headers: {
                'Authorization': `ApiKey ${apiKey}`
            }
        })
    ...
    const handleFollowFeed = (feedId) => {
        if (window.confirm('Are you sure you want to follow this feed?')) {
            const apiKey = localStorage.getItem('apiKey');
            console.log(apiKey);
            axios.post('https://192.168.239.148:8080/v1/feed_follows', { feed_id: feedId }, {
                headers: {
                    'Authorization': `ApiKey ${apiKey}`
                }
            })
                .then(response => {
                    ...
                    axios.get(`https://192.168.239.148:8080/v1/posts?feed_id=${feedId}`, {
                        headers: {
                            'Authorization': `ApiKey ${apiKey}`
                        }
                    })
    ...
    const handleUnfollowFeed = (feedId) => {
        if (window.confirm('Are you sure you want to unfollow this feed?')) {
            const apiKey = localStorage.getItem('apiKey');
            axios.delete(`https://192.168.239.148:8080/v1/feed_follows/${followedFeedFollowIds[feedId]}`, {
                headers: {
                    'Authorization': `ApiKey ${apiKey}`
                }
            })
